	...
	public double[] run(AbstractProblem funct, int maxEvaluations) {
		...
		while(currentEvaluations < maxEvaluations) { //Until stopping condition is met
			for(int i = 0; i < POPSIZE; i++) { //For each individual in Pop
				...
				double childFitness = funct.f(child) + (maxViolation[maxViolation.length-1] * ADDPENALTYCOEFF);
				currentEvaluations++;
				maxViolation = funct.violationArray(parent);
				Arrays.sort(maxViolation);
				double parentFitness = funct.f(parent) + (maxViolation[maxViolation.length-1] * ADDPENALTYCOEFF);
				currentEvaluations++;
