        ...
        while (currentEvaluations < maxEvaluations) { //Until stopping condition is met
            for (int i = 0; i < POPSIZE; i++) { //For each individual in Pop
                ...
                for (int j = 0; j < POPSIZE; j++) {
                    if(problem.f(CurrentPopulationPx.get(j)) > problem.f(bestIndividual)){
                ...
                //Replacement
                double trialVectorFitness = problem.f(trialVector);
                currentEvaluations++;
                double targetVectorFitness = problem.f(targetVector);
                currentEvaluations++;
