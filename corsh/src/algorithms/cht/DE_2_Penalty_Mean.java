...
public class DE_2_Penalty_Mean extends AbstractAlgorithm {
	...
	public Solution run(AbstractProblem funct, int maxEvaluations) {
		...
		while(currentEvaluations < maxEvaluations) { //Until stopping condition is met
			for(int i = 0; i < POPSIZE; i++) { //For each individual in Pop
				...
				//Replacement
				double childFitness = funct.fitness(child) + (funct.violation(child) * ADDPENALTYCOEFF);
				currentEvaluations++;
				double parentFitness = funct.fitness(parent) + (funct.violation(parent) * ADDPENALTYCOEFF);
				currentEvaluations++;
