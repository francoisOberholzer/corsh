...
public class DE_6_PieceWise_Epsilon extends AbstractAlgorithm {
	...
	public Solution run(AbstractProblem funct, int maxEvaluations) {
		...
		int Tc = (int) ((maxEvaluations/200) * 0.8); //Generation at which epsilon becomes 0
		...
		while(currentEvaluations < maxEvaluations) { //Until stopping condition is met
			for(int i = 0; i < POPSIZE; i++) { //For each individual in Pop
				...
				if(((childViolation <= epsilon) && (parentViolation <= epsilon)) || (childViolation == parentViolation)) { //Both Valid OR equally invalid, take best fitness
	...
	private static double getEpsilon(AbstractProblem funct, ArrayList<Vector<Double>> population, int generation, int Tc) {
		double[] popViolations = new double[POPSIZE];
		int theta = (int) (0.8*POPSIZE);
		double epsilon;
		
		for(int i = 0; i < POPSIZE; i++) {
			popViolations[i] = funct.violation(population.get(i));
		}
		Arrays.sort(popViolations);
		
		epsilon = popViolations[theta] * Math.pow(1 - (generation/Tc), cp);
		
		return epsilon;
	}
	
	public String getName() { 
		return "DE_PieceWiseEpsilon";
	}
}
